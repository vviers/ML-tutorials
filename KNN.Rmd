...
In order to implement our own version of KNN, we need to decide on a notion of distance. A very often used distance between two points $u = (x_u, y_u)$ and $v = (x_v, y_v)$ is the *Euclidian Distance* which you probably know from high-school geometry and Pythagora's theorem:

$$eucl\_distance(u, v) =  \sqrt{(x_u - x_v)^2 + (y_u - y_v)^2} $$
...
With this function in place, we are now ready to implement our classifier which:
    1. takes in a new data point `new_vec` and the number of nearest neighbours to consider `k`,
    1. computes the distance between `new_vec` and all other points in the training set,
    1. orders the nearest points by (increasing) distance,
    1. selects the `k` nearest points, and
    1. returns its prediction for the label of `new_vec` (-1 or 1).
